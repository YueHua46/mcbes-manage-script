...
class PlayerMarketForm {
  ...
  browseItems(player: Player, page: number = 1): void {
    ...
    const chestForm = new ChestFormData("shop").title(`玩家交易市场 - 所有商品 (第${page}/${totalPages}页)`);
  ...
  myListedItems(player: Player, page: number = 1): void {
    const items = playerMarket.getPlayerItems(player.name);
   ...
   * 显示物品详情
   */
  private showItemDetails(player: Player, item: MarketItem, page: number): void {
    const form = new ActionFormData()
      .title("商品详情")
      .body(
    ...
    form.show(player).then((response) => {
      ...
      if (response.selection === 0) {
        if (item.data.amount > 1) {
          ...
          this.confirmMarketPurchase(player, item, 1, page);
        }
   ...
   * 管理员商品管理
   */
  private showAdminItemDetails(player: Player, item: MarketItem, page: number): void {
    if (!isAdmin(player)) {
  ...
  private showAdminSetPriceForm(player: Player, item: MarketItem, page: number): void {
    ...
    modal.show(player).then((res) => {
      ...
      const result = playerMarket.adminSetItemPrice(player, item, newPrice);
  ...
  private confirmAdminReturnItem(player: Player, item: MarketItem, page: number): void {
  ...
  private confirmAdminConfiscateItem(player: Player, item: MarketItem, page: number): void {
    openConfirmDialogForm(
      player,
      "没收商品",
      `${colorCodes.red}确定要没收 ${colorCodes.aqua}${item.data.name}${colorCodes.red} 吗？\n${colorCodes.white}物品会从玩家交易市场移除，并进入你的背包。`,
      () => {
        void playerMarket
   ...
   * 询问购买数量
   */
  private askBuyQuantity(player: Player, item: MarketItem, page: number): void {
    ...
    modal.show(player).then((res) => {
      ...
      this.confirmMarketPurchase(player, item, qty, page);
    });
  }
   ...
   */
  private confirmMarketPurchase(player: Player, item: MarketItem, qty: number, page: number): void {
   ...
   * 显示我的商品详情
   */
  private showMyItemDetails(player: Player, item: MarketItem, page: number = 1): void {
    const form = new ActionFormData()
      .title("我的商品")
      .body(
        `${colorCodes.gold}商品名称: ${colorCodes.white}${item.data.name}\n` +
    ...
    form.show(player).then((response) => {
      ...
      if (response.selection === 0) {
        playerMarket.unlistItem(player, item, () => this.myListedItems(player, page));
      } else {
   ...
   * 显示购买确认
   */
  private showBuyConfirmation(player: Player, item: MarketItem, qty: number, page: number): void {
    ...
    form.show(player).then((response) => {
      ...
      void playerMarket.buyItem(player, item, qty, () => this.browseItems(player, page));
    });
  }
  ...
  private showItemListingForm(player: Player, item: ItemStack, slot: number): void {
    ...
    form.show(player).then((response) => {
      ...
      playerMarket.listItem(player, slot, price, amount, name, desc, () => this.openMainMenu(player));
    });
  }
}

const playerMarketForm = new PlayerMarketForm();
export default playerMarketForm;
