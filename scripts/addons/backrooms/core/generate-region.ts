import { BackroomsGenerationConfig, createBackroomsConfig } from "./config";
  ...
  for (let across = gate.offset; across < gate.offset + gate.width; across += 1) {
    for (let layer = 0; layer < borderThickness; layer += 1) {
      switch (gate.direction) {
        case "north":
          grid.set(across, layer, BackroomsCell.Gate);
          break;
        case "east":
          grid.set(grid.size - 1 - layer, across, BackroomsCell.Gate);
          break;
        case "south":
          grid.set(across, grid.size - 1 - layer, BackroomsCell.Gate);
          break;
        case "west":
          grid.set(layer, across, BackroomsCell.Gate);
          break;
      }
    }
  }
  ...
  for (let z = 1; z < grid.size - 1; z += 1) {
    for (let x = 1; x < grid.size - 1; x += 1) {
      ...
      while (
        ...
        grid.get(maxX + 1, z) === BackroomsCell.Wall
      )
        maxX += 1;
...
export function findSafeLandingCell(layout: RegionLayout, target?: { x: number; z: number }): SafeLandingCell {
  ...
  for (let z = 2; z < layout.size - 3; z += 1) {
    for (let x = 2; x < layout.size - 3; x += 1) {
      if (
        layout.grid.get(x, z) !== BackroomsCell.Walkable ||
        layout.grid.get(x + 1, z) !== BackroomsCell.Walkable ||
        layout.grid.get(x, z + 1) !== BackroomsCell.Walkable ||
        layout.grid.get(x + 1, z + 1) !== BackroomsCell.Walkable
      )
        continue;
...
export function generateRegionLayout(
  ...
  const layoutRandom = new DeterministicRandom(
    deriveSeed32(worldSeed, config.algorithmVersion, "bsp-layout", coordinate.rx, coordinate.rz)
  );
