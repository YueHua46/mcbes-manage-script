...
import { BackroomsCell, ConnectivityRepair, ConnectivityReport, LocalCoordinate, RegionGrid } from "./types";
...
class MinimumHeap {
  ...
  public pop(): { index: number; distance: number } | undefined {
    ...
    while (true) {
      ...
      const right = left + 1;
      const child = right < this.nodes.length && this.nodes[right].distance < this.nodes[left].distance ? right : left;
...
function carveRepairCorridor(grid: RegionGrid, path: readonly number[], width: number): number {
...
export function ensureConnectedLayout(grid: RegionGrid, config: BackroomsGenerationConfig): ConnectivityReport {
  ...
  while (components.length > 1) {
    ...
    const gateComponent = components.find((component) =>
      component.some((index) => grid.getByIndex(index) === BackroomsCell.Gate)
    );
    const source =
      gateComponent !== undefined
        ? gateComponent
