...
function openAddCategoryForm(player: Player) {
  ...
  form.show(player).then((data) => {
    ...
    const { formValues } = data;
    console.error(`formValues[2] : ${formValues?.[2]}`);
    console.error(`target icon : ${glyphList[Number(formValues?.[2])]}`);
    if (formValues?.[0] && formValues?.[1]) {
      const result = productCategory.createCategory({
        name: formValues[0].toString(),
        description: formValues[1]?.toString(),
        icon: glyphKeys[formValues?.[2] as number] || productCategory.defaultIcon,
...
// 删除商品类别表单
export function openDeleteCategoryForm(player: Player) {
  const form = new ActionFormData();
  form.title("§w删除商品类别");
  ...
  form.show(player).then((data) => {
    ...
    // 选择了某个类别进行删除
    const selectedCategory = categories[data.selection as number];

    // 确认删除
    openConfirmDialogForm(
      player,
      "确认删除",
      `${color.yellow("确定要删除商品类别")} ${color.green(selectedCategory.name)} ${color.yellow(
        "吗？此操作不可撤销，该类别下的所有商品也将被删除！"
      )}`,
      () => {
        // 执行删除操作
...
// 编辑商品类别表单
function openEditCategoryForm(player: Player, categoryName: string) {
  ...
  form.dropdown("§w请选择类别图标", glyphList, {
    defaultValueIndex: glyphList.indexOf(category.icon as string),
...
// 商品类别详情管理表单
function openCategoryDetailForm(player: Player, categoryName: string) {
  const category = productCategory.getCategory(categoryName);
  if (!category) {
    return openDialogForm(
      player,
...
// 商品管理表单
function openProductManageForm(player: Player, categoryName: string) {
  const category = productCategory.getCategory(categoryName);
  if (!category) {
    return openDialogForm(
      player,
...
// 添加商品表单
function openAddProductForm(player: Player, categoryName: string) {
  const category = productCategory.getCategory(categoryName);
  if (!category) {
    return openDialogForm(
      player,
...
// 删除商品表单
function openDeleteProductForm(player: Player, categoryName: string, page: number = 1) {
  const category = productCategory.getCategory(categoryName);
  if (!category) {
    return openDialogForm(
      player,
  ...
  chestForm.show(player).then((response) => {
    ...
    if (selection !== undefined && selection < currentPageProducts.length) {
      ...
      openConfirmDialogForm(
        player,
        "删除确认",
        `${color.red("确定要删除商品")} ${color.green(selectedProduct.name)} ${color.red("吗？")}`,
        () => {
          // 执行删除操作
...
// 商品类别管理表单
function openCategoryManageDetailForm(player: Player, categoryName: string) {
  const category = productCategory.getCategory(categoryName);
  if (!category) {
    return openDialogForm(
      player,
...
// 打开商品类别列表管理表单
function openCategoryListSettingForm(player: Player, page: number = 1) {
  const form = new ActionFormData();
  ...
  currentPageCategories.forEach((category) => {
    form.button(
      `§w${category.name}\n§7${category.description || "无描述"}`,
      emojiKeyToEmojiPath(category.icon!) || "textures/icons/quest"
    );
  });
...
// 商品类别管理主菜单
function openCategoryManageForm(player: Player) {
  const form = new ActionFormData();
  form.title("§w商品类别管理");
