...
world.afterEvents.worldLoad.subscribe(() => {
  system.run(() => {
    ...
    const current = dimensionRegistry.getRegisteredDimension(BACKROOMS_ALIAS);
    if (!current?.spawn) {
      dimensionRegistry.setRegisteredDimensionSpawn(BACKROOMS_ALIAS, BACKROOMS_SPAWN);
    }
    startRuntime();
  });
});
