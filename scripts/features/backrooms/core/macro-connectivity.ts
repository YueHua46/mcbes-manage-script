...
export function isMandatoryRegionConnection(
    ...
    return sameRegion(getRegionParent(seed, a, config), b)
        || sameRegion(getRegionParent(seed, b, config), a);
}
...
export function getEdgeGate(
    ...
    const width = wideOpening
        ? 9 + (widthHash % 7)
        : config.gateWidths[widthHash % config.gateWidths.length];
