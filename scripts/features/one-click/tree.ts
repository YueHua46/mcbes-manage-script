/**
 * 一键砍树功能
 * 完整迁移自 Modules/OneClick/Tree.ts (210行)
 */
...
function consumeAxeDurability(player: Player, logLocations: Vector3[]): void {
...
function getLogLocations(dimension: Dimension, location: Vector3, currentBreakBlockTypeId: string): Vector3[] {
...
async function treeCut(location: Vector3, dimension: Dimension, logLocations: Vector3[]): Promise<void> {
...
world.afterEvents.playerBreakBlock.subscribe(async (e) => {
  try {
    if (!setting.getState("enableTreeCutOneClick")) return;
