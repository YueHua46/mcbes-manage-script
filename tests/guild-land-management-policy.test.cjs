...
test("guild land officers manage routine settings while owner keeps destructive actions", () => {
  ...
  assert.match(landForm, /if \(canManageGuildLand\)[\s\S]*text: "领地公开权限"[\s\S]*text: "领地快照"/);
...
test("guild officers cannot change the server-wide snapshot chunk limit", () => {
  ...
  assert.match(snapshotForm, /canManageLandSnapshots/);
});
