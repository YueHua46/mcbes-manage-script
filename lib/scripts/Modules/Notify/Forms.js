...
export const openAddNotifyForm = (player) => {
    ...
    form.show(player).then((data) => {
        ...
        if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[1])) {
            ...
            openDialogForm(player, { title: "添加失败", desc: color.red("表单未填写完整，请填写完整！") }, () => openAddNotifyForm(player));
    });
};
export const openDeleteNotifyForm = (player) => {
    ...
    form.dropdown("选择通知", notifys.map((n) => n.title));
    form.submitButton("确定");
    form.show(player).then((data) => {
        if (data.cancelationReason)
            return;
        ...
        if (typeof data.selection === "number") {
            ...
            form.show(player).then((data) => {
                ...
                if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[1])) {
                    ...
                    openDialogForm(player, { title: "更新失败", desc: color.red("表单未填写完整，请填写完整！") }, () => openUpdateNotifyForm(player));
            });
        }
