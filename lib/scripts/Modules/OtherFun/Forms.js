...
export function openBaseFunctionForm(player) {
    ...
    const backToDeath = setting.getState("backToDeath");
    ...
    buttons.push({ text: "§w自杀", icon: "textures/ui/bad_omen_effect", action: () => player.kill() });
    // 根据设置决定是否添加回到死亡地点按钮
    if (backToDeath) {
        buttons.push({
            text: "§w回到上次死亡地点",
            ...
            action: () => {
                ...
                if (deathData === null || deathData === void 0 ? void 0 : deathData.length) {
    ...
    buttons.push({ text: "§w服务器状态", icon: "textures/ui/servers", action: () => openServerInfoForm(player) });
    form.title("§w其他功能");
    ...
    form.show(player).then((data) => {
        ...
        if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[1])) {
            if (formValues[0].toString().length > 8)
                return openDialogForm(player, { title: "添加失败", desc: color.red("标题长度不能超过8个字符！") }, () => openAddLeaveMessageForm(player));
            leaveMessage.createLeaveMessage({
                title: formValues[0].toString(),
                content: formValues[1].toString(),
                creator: player.name,
            });
            openDialogForm(player, { title: "添加成功", desc: color.green("留言添加成功！") }, () => openLeaveMessageForms(player));
        }
        else {
            openDialogForm(player, { title: "添加失败", desc: color.red("表单未填写完整，请填写完整！") }, () => openAddLeaveMessageForm(player));
        }
    });
};
// 删除留言
export const openDeleteLeaveMessageForm = (player, isAdmin = false) => {
    ...
    form.dropdown("选择留言", lms.map((lm) => lm.title));
    form.submitButton("§w确定");
    form.show(player).then((data) => {
        if (data.cancelationReason)
            return;
        const { formValues } = data;
        if (typeof (formValues === null || formValues === void 0 ? void 0 : formValues[0]) === "number") {
            leaveMessage.deleteLeaveMessage(lms[Number(formValues[0])].id);
            openDialogForm(player, { title: "删除成功", desc: color.green("留言删除成功！") }, () => openLeaveMessageForms(player));
        }
    });
};
