...
function teleportPlayer(requestPlayer, targetPlayer, type) {
    if (type === "to") {
        ...
        requestPlayer.sendMessage(`${color.green("你已")}${color.green("传送到")} ${color.yellow(targetPlayer.name)} ${color.green("的旁边")}`);
        targetPlayer.sendMessage(`${color.green("玩家")} ${color.yellow(requestPlayer.name)} ${color.green("已传送到你的旁边")}`);
    }
    else {
    ...
    form.show(targetPlayer).then((data) => {
        ...
        switch (data.selection) {
            case 0:
                ...
                break;
            case 1:
                requestPlayer.sendMessage(`${color.red("玩家")} ${color.yellow(targetPlayer.name)} ${color.red("拒绝了你的传送请求")}`);
                targetPlayer.sendMessage(`${color.red("你已")}${color.red("拒绝了")} ${color.yellow(requestPlayer.name)} ${color.red("的传送请求")}`);
                break;
        }
    });
}
...
function createPlayerTpaForm(allPlayer) {
    ...
    form.title(`${"玩家传送"}`);
    form.dropdown("§w选择玩家", allPlayer.map((player) => ` ${player.name}`));
...
export function openPlayerActionForm(player) {
    ...
    form.show(player).then((data) => {
        if (data.cancelationReason || data.canceled)
            return;
        switch (data.selection) {
            case 0:
...
export function openDeleteChatBlackListForm(player) {
    ...
    const _blackList = JSON.parse(blackList !== null && blackList !== void 0 ? blackList : "[]");
...
export function openAddChatBlackListForm(player) {
    ...
    form.dropdown("§w选择对应玩家", allPlayers.map((p) => p.name));
    form.submitButton("§w确认");
    form.show(player).then((data) => {
        if (data.cancelationReason || data.canceled)
            return;
