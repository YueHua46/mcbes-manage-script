...
import { emojiKeyToEmojiPath, SystemLog } from "../../utils/utils";
import WayPoint from "../WayPoint/WayPoint";
import { defaultSetting } from "./Setting";
import productCategory from "../Economic/ProcuctCategory";
...
export const openFunctionSwitchForm = (player) => {
    ...
    const buttons = [
        ...
        },
        {
            text: "§w随机传送",
            id: "randomTeleport",
            state: (_g = setting.getState("randomTeleport")) !== null && _g !== void 0 ? _g : true,
        },
        {
            text: "§w回到死亡点",
            id: "backToDeath",
            state: (_h = setting.getState("backToDeath")) !== null && _h !== void 0 ? _h : true,
        },
    ];
    form.title("§w功能开关");
...
export const openSystemSettingForm = (player) => {
    ...
    const buttons = [
        ...
        },
        {
            ...
            action: () => openCategoryManageForm(player),
        },
        {
            text: "通用系统设置",
...
// 领地设置主表单
export const openLandSettingsForm = (player) => {
    const form = new ActionFormData();
    form.title("§w领地设置");
    const buttons = [
        {
...
export const openOneClickMineTreeSettingForm = (player) => {
    const form = new ModalFormData();
...
// 经济系统相关
// 添加商品类别表单
function openAddCategoryForm(player) {
    const form = new ModalFormData();
    form.title("§w添加商品类别");
    form.textField("§w类别名称", "请输入类别名称(如: 食物、武器、材料)", {
        defaultValue: "",
        tooltip: "类别名称不能重复，请注意",
    });
    form.textField("§w类别描述", "请输入类别描述", {
        defaultValue: "",
        tooltip: "类别描述用于描述该类别商品的特点",
    });
    form.dropdown("§w选择图标", glyphList);
    ...
    form.show(player).then((data) => {
        ...
        if (data.canceled || data.cancelationReason)
            return;
        const { formValues } = data;
        console.error(`formValues[2] : ${formValues === null || formValues === void 0 ? void 0 : formValues[2]}`);
        console.error(`target icon : ${glyphList[Number(formValues === null || formValues === void 0 ? void 0 : formValues[2])]}`);
        if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[1])) {
            const result = productCategory.createCategory({
                name: formValues[0].toString(),
                description: (_a = formValues[1]) === null || _a === void 0 ? void 0 : _a.toString(),
                icon: glyphKeys[formValues === null || formValues === void 0 ? void 0 : formValues[2]] || productCategory.defaultIcon,
...
// 删除商品类别表单
export function openDeleteCategoryForm(player) {
    const form = new ActionFormData();
    form.title("§w删除商品类别");
    ...
    if (categories.length === 0) {
        return openDialogForm(player, {
            title: "删除失败",
            desc: color.red("没有可删除的商品类别！"),
        }, () => openSystemSettingForm(player));
    }
    categories.forEach((category) => {
        form.button(`§w${category.name}\n§7${category.description || "无描述"}`, emojiKeyToEmojiPath(category.icon) || emojiKeyToEmojiPath(productCategory.defaultIcon));
    });
    form.button("§w返回", "textures/icons/back");
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return;
        if (data.selection === categories.length) {
            // 点击了返回按钮
            openSystemSettingForm(player);
            return;
        }
        // 选择了某个类别进行删除
        const selectedCategory = categories[data.selection];
        // 确认删除
        openConfirmDialogForm(player, "确认删除", `${color.yellow("确定要删除商品类别")} ${color.green(selectedCategory.name)} ${color.yellow("吗？此操作不可撤销，该类别下的所有商品也将被删除！")}`, () => {
            // 执行删除操作
            const result = productCategory.deleteCategory(selectedCategory.name);
            if (result) {
                openDialogForm(player, {
                    title: "删除成功",
                    desc: color.green(`商品类别 ${color.yellow(selectedCategory.name)}${color.green(" 已成功删除！")}`),
                }, () => openCategoryManageForm(player));
            }
            else {
                openDialogForm(player, {
                    title: "删除失败",
                    desc: color.red(`删除商品类别 ${color.green(selectedCategory.name)} ${color.red("失败！")}`),
                }, () => openCategoryManageForm(player));
            }
        });
    });
...
// 编辑商品类别表单
function openEditCategoryForm(player, categoryName) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
            desc: color.red("商品类别不存在！"),
        }, () => openCategoryListSettingForm(player));
    }
    const form = new ModalFormData();
    form.title("§w编辑商品类别");
    form.textField("§w类别名称", "请输入类别名称", {
    ...
    form.dropdown("§w请选择类别图标", glyphList, {
        defaultValueIndex: glyphList.indexOf(category.icon),
        tooltip: "类别图标用于显示在菜单中，直观的了解类别",
    });
    ...
    form.show(player).then((data) => {
        ...
        const { formValues } = data;
        if (formValues === null || formValues === void 0 ? void 0 : formValues[0]) {
            const newName = formValues[0].toString();
            ...
            if (typeof result === "string") {
                openDialogForm(player, {
                    title: "修改失败",
                    desc: color.red(result),
                }, () => openEditCategoryForm(player, categoryName));
            }
            else {
                openDialogForm(player, {
                    title: "修改成功",
                    desc: color.green("商品类别修改成功！"),
                }, () => openCategoryDetailForm(player, newName));
            }
        }
        else {
            openDialogForm(player, {
...
// 商品类别详情管理表单
function openCategoryDetailForm(player, categoryName) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
            desc: color.red("商品类别不存在！"),
        }, () => openCategoryDetailForm(player, categoryName));
    }
    const form = new ActionFormData();
    form.title("§w商品类别详情");
    form.body(`§a类别名称: §e${category.name}\n` +
        `§a类别描述: §e${category.description || "无"}\n` +
        `§a创建时间: §e${category.created}\n` +
        `§a修改时间: §e${category.modified}\n` +
        `§a创建者: §e${category.createdBy || "未知"}`);
    form.button("§w商品管理", "textures/packs/031-gift-box-1");
    form.button("§w商品类别管理", "textures/packs/035-cyclone");
    form.button("§w返回", "textures/icons/back");
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return;
        switch (data.selection) {
            case 0:
                // 商品管理
                openProductManageForm(player, categoryName);
                break;
            case 1:
                // 商品类别管理
                openCategoryManageDetailForm(player, categoryName);
                break;
            case 2:
                openCategoryListSettingForm(player);
                break;
...
// 商品管理表单
function openProductManageForm(player, categoryName) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
    ...
    const form = new ActionFormData();
    form.title(`§w${category.name} - 商品管理`);
    form.button("§w添加商品", "textures/icons/add");
    form.button("§w删除商品", "textures/icons/deny");
    form.button("§w返回", "textures/icons/back");
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return;
        switch (data.selection) {
            case 0:
                // 添加商品
                openAddProductForm(player, categoryName);
                break;
            case 1:
                // 删除商品
                openDeleteProductForm(player, categoryName);
                break;
            case 2:
                openCategoryDetailForm(player, categoryName);
                break;
...
// 添加商品表单
function openAddProductForm(player, categoryName) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
    ...
    const form = new ModalFormData();
    form.title(`§w添加商品 - ${category.name}`);
    form.textField("§w商品名称", "请输入商品名称", {
        defaultValue: "",
        tooltip: "商品名称不能重复，请注意",
    });
    form.textField("§w商品描述", "请输入商品描述", {
        defaultValue: "",
        tooltip: "商品描述用于描述该商品的特点",
    });
    form.textField("§w商品对应itemId", "商品对应的itemId,如 minecraft:dimond", {
        defaultValue: "",
        tooltip: "商品对应的itemId,如 minecraft:dimond",
    });
    form.textField("§w商品价格", "请输入商品单价", {
        defaultValue: "10",
        tooltip: "商品单价，必须为正整数",
    });
    form.textField("§w商品供应量", "请输入商品总供应量", {
        defaultValue: "100",
        tooltip: "商品总供应量，必须为正整数",
    });
    form.submitButton("§w确认添加");
    form.show(player).then((data) => {
        var _a, _b;
        if (data.canceled || data.cancelationReason)
            return;
        const { formValues } = data;
        if (formValues && (formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[2])) {
            const productName = formValues[0].toString();
            const productDescription = (_b = (_a = formValues[1]) === null || _a === void 0 ? void 0 : _a.toString()) !== null && _b !== void 0 ? _b : "";
            const productItemId = formValues[2].toString();
            const productPrice = parseInt(formValues[3].toString());
            const productStock = parseInt(formValues[4].toString());
            if (isNaN(productPrice) || productPrice <= 0 || isNaN(productStock) || productStock <= 0) {
                return openDialogForm(player, {
                    title: "添加失败",
                    desc: color.red("商品价格和商品供应量必须是大于0的数字！"),
                });
            }
            const result = productCategory.addProductToCategory(categoryName, {
                name: productName,
                icon: productItemId,
            ...
            openDialogForm(player, {
                title: "添加失败",
                desc: color.red("商品商品名称、商品itemId是必填的！"),
            }, () => openAddProductForm(player, categoryName));
        }
    });
}
// 删除商品表单
function openDeleteProductForm(player, categoryName, page = 1) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
    ...
    if (!products || products.length === 0) {
        return openDialogForm(player, {
            title: "删除失败",
            desc: color.red("该类别下暂无商品！"),
        }, () => openProductManageForm(player, categoryName));
    }
    // 创建ChestUI表单显示所有商品
    const chestForm = new ChestFormData("shop");
    chestForm.title(`${category.name} - 删除商品`);
    // 计算分页信息
    const pageSize = 45; // 箱子UI每页最多显示45个物品
    ...
    for (let i = 0; i < currentPageProducts.length; i++) {
        ...
        chestForm.button(i, product.name, [`价格: ${product.price} 金币/个`, "点击删除"], product.itemId, 1);
    }
    // 添加导航按钮
    if (page > 1) {
    ...
    if (page < totalPages) {
        chestForm.button(46, "下一页", ["查看下一页商品"], "textures/icons/right_arrow", 1);
    }
    // 添加返回按钮
    chestForm.button(49, "返回", ["返回商品管理"], "textures/icons/back", 1);
    // 显示ChestUI并处理删除逻辑
    chestForm.show(player).then((response) => {
        ...
        if (selection === 45 && page > 1) {
            // 点击了上一页
            openDeleteProductForm(player, categoryName, page - 1);
            return;
        }
        if (selection === 46 && page < totalPages) {
            // 点击了下一页
            openDeleteProductForm(player, categoryName, page + 1);
            return;
        }
        ...
        if (selection !== undefined && selection < currentPageProducts.length) {
            ...
            // 弹窗确认是否删除
            openConfirmDialogForm(player, "删除确认", `${color.red("确定要删除商品")} ${color.green(selectedProduct.name)} ${color.red("吗？")}`, () => {
                // 执行删除操作
                const result = productCategory.removeProductFromCategory(categoryName, selectedProduct.name);
                if (typeof result === "string") {
                    openDialogForm(player, {
                        title: "删除失败",
                        desc: color.red(result),
                    }, () => openDeleteProductForm(player, categoryName, page) // 删除失败返回删除商品表单
                    );
                }
                else {
                    openDialogForm(player, {
                        title: "删除成功",
                        desc: color.green("商品删除成功！"),
                    }, () => openDeleteProductForm(player, categoryName, page) // 删除成功后返回最新的商品列表
                    );
                }
            });
        }
...
// 商品类别管理表单
function openCategoryManageDetailForm(player, categoryName) {
    const category = productCategory.getCategory(categoryName);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
    ...
    const form = new ActionFormData();
    form.title(`§w${category.name} - 类别管理`);
    form.button("§w编辑商品类别", "textures/icons/edit2");
    form.button("§w删除商品类别", "textures/icons/deny");
    form.button("§w返回", "textures/icons/back");
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return;
        switch (data.selection) {
            ...
            case 1:
                // 删除商品类别
                openConfirmDialogForm(player, "删除确认", `${color.red("确定要删除商品类别")} ${color.green(category.name)} ${color.red("吗？")}`, () => {
                    const result = productCategory.deleteCategory(categoryName);
                    if (typeof result === "string") {
                        openDialogForm(player, {
                            title: "删除失败",
                            desc: color.red(result),
...
// 打开商品类别列表管理表单
function openCategoryListSettingForm(player, page = 1) {
    const form = new ActionFormData();
    form.title("§w商品类别列表管理");
    const categories = productCategory.getCategories();
    const pageSize = 10;
    ...
    currentPageCategories.forEach((category) => {
        form.button(`§w${category.name}\n§7${category.description || "无描述"}`, emojiKeyToEmojiPath(category.icon) || "textures/icons/quest");
    });
    ...
    form.show(player).then((data) => {
        ...
        if (selectionIndex < currentPageCategories.length) {
            ...
            // 上一页
            openCategoryListSettingForm(player, page - 1);
        }
        else if (selectionIndex === nextButtonIndex - 1 && page < totalPages) {
            // 下一页
            openCategoryListSettingForm(player, page + 1);
...
// 商品类别管理主菜单
function openCategoryManageForm(player) {
    const form = new ActionFormData();
    form.title("§w商品类别管理");
    const buttons = [
        {
            text: "§w查看所有商品类别",
            icon: "textures/packs/14321660",
            action: () => openCategoryListSettingForm(player),
        },
        {
            text: "§w添加商品类别",
        ...
        },
        {
            text: "§w返回",
            icon: "textures/icons/back",
            action: () => openSystemSettingForm(player),
        },
    ];
    buttons.forEach(({ text, icon }) => form.button(text, icon));
    form.show(player).then((data) => {
