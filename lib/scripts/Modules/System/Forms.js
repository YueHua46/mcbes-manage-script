...
export const openTrialModeSettingForm = (player) => {
    ...
    form.textField("§w试玩时长(秒) 玩家在线达到此时长后才会自动转为生存模式，否则将一直是冒险模式", "请输入试玩时长(秒)", duration.toString());
    ...
    form.show(player).then((data) => {
        ...
        if (formValues) {
            ...
            openDialogForm(player, { title: "设置成功", desc: color.green("试玩模式设置已保存!") }, () => openCommonSettingForm(player));
        }
    });
};
