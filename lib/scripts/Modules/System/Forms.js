...
import { isNumber, SystemLog, toNumber } from "../../utils/utils";
...
// 打开创建领地每方块价格设置表单
export const openCreateLand1BlockPerPrice = (player) => {
    const form = new ModalFormData();
    ...
    form.show(player).then((data) => {
        ...
        const { formValues } = data;
        // 存在且可转换为数字
        if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && !isNaN(toNumber(formValues === null || formValues === void 0 ? void 0 : formValues[0]))) {
            const price = formValues[0].toString();
            setting.setState("land1BlockPerPrice", price);
            openDialogForm(player, { title: "创建领地每方块价格设置成功", desc: color.green("创建领地每方块价格设置成功！") }, () => openSystemSettingForm(player));
        }
    });
};
// 打开服务器名称设置表单
...
// 添加会员管理表单
export const openMemberManageForm = (player) => {
    const form = new ActionFormData();
    form.title("§w会员管理");
...
// 添加会员表单
export const openAddMemberForm = (player) => {
    const form = new ModalFormData();
    form.title("§w添加会员");
...
// 移除会员表单
export const openRemoveMemberForm = (player) => {
    const form = new ModalFormData();
    form.title("§w移除会员");
...
// 会员列表表单
export const openMemberListForm = (player, page = 1) => {
    const form = new ActionFormData();
    form.title("§w会员列表");
...
// 经济系统管理
export const openEconomyMenuForm = (player) => {
    const form = new ActionFormData();
    form.title("§w经济系统管理");
...
// 设置玩家金币数量表单
export const openSetPlayerMoneyForm = (player) => {
    const playerList = world.getPlayers();
    const form = new ModalFormData();
    form.title("§w管理玩家金币数量");
    form.dropdown("§w玩家选择（和玩家名称二选一即可）", playerList.map((player) => ` ${player.name}`), {
        defaultValueIndex: 0,
    ...
    form.show(player).then((data) => {
        ...
        if (data.canceled || data.cancelationReason)
            return;
        const { formValues } = data;
        console.warn(`formValues: ${formValues === null || formValues === void 0 ? void 0 : formValues[0]}, ${formValues === null || formValues === void 0 ? void 0 : formValues[1]}, ${formValues === null || formValues === void 0 ? void 0 : formValues[2]}, ${formValues === null || formValues === void 0 ? void 0 : formValues[3]}`);
        if (formValues && (isNumber(formValues === null || formValues === void 0 ? void 0 : formValues[0]) || formValues[1]) && isNumber(formValues[2])) {
            const playerIndex = formValues[0];
            const playerName = (_a = formValues[1]) === null || _a === void 0 ? void 0 : _a.toString();
            const operationType = formValues[2];
            const amount = parseInt(((_b = formValues[3]) === null || _b === void 0 ? void 0 : _b.toString()) || "0");
            ...
            console.warn(`playerIndex: ${playerIndex}, playerName: ${playerName}, operationType: ${operationType}, amount: ${amount}`);
            if (targetPlayer) {
