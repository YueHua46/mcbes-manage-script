...
function openEditCategoryForm(player, categoryName) {
    ...
    form.dropdown("§w请选择类别图标", glyphList, glyphList.indexOf(category.icon));
...
function openAddProductForm(player, categoryName) {
    ...
    form.textField("§w商品描述", "请输入商品描述", "该商品无描述");
    form.textField("§w商品对应itemId", "商品对应的itemId,如 minecraft:dimond", "");
    form.textField("§w商品价格", "请输入商品单价", "10");
    form.textField("§w商品供应量", "请输入商品总供应量", "1000");
    form.submitButton("§w确认添加");
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return;
        const { formValues } = data;
        if ((formValues === null || formValues === void 0 ? void 0 : formValues[0]) && (formValues === null || formValues === void 0 ? void 0 : formValues[2])) {
            const productName = formValues[0].toString();
            const productDescription = formValues[1].toString();
