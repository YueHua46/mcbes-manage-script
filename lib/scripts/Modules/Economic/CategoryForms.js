...
// 打开商品类别列表表单
export function openCategoryListForm(player, page = 1) {
...
function openChestShopByCategoryForm(player, categoryName, page = 1) {
    ...
    if (!products || products.length === 0) {
        return openDialogForm(player, {
            title: "打开失败",
            desc: color.red(`§c类别 ${color.yellow(categoryName)} ${color.red("下不存在任何商品！请联系管理员添加商品。")}`),
        }, () => openCategoryListForm(player));
    }
    // 计算分页
...
// 打开购买数量选择表单
function openBuyQuantityForm(player, product, categoryName, page) {
    ...
    form.show(player).then((data) => {
        if (data.canceled || data.cancelationReason)
            return openChestShopByCategoryForm(player, categoryName, page);
...
// 打开确认购买表单
function openConfirmPurchaseForm(player, product, quantity, totalPrice, categoryName, page) {
    const form = new ActionFormData();
    form.title("确认购买");
...
// 处理购买逻辑
function processPurchase(player, product, quantity, totalPrice, categoryName, page) {
    // 检查玩家余额
