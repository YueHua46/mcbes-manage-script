...
// 商品类别管理主菜单
export function openCategoryManageForm(player) {
...
// 删除商品类别表单
function openDeleteCategoryForm(player) {
    const form = new ActionFormData();
    form.title("§w删除商品类别");
...
// 商品类别详情表单
function openCategoryDetailForm(player, categoryId) {
...
// 编辑商品类别表单
function openEditCategoryForm(player, categoryId) {
    const category = productCategory.getCategory(categoryId);
    if (!category) {
        return openDialogForm(player, {
            title: "错误",
    ...
    form.show(player).then((data) => {
        ...
        if (formValues === null || formValues === void 0 ? void 0 : formValues[0]) {
            const result = productCategory.updateCategory({
                name: categoryId,
                description: (_a = formValues[1]) === null || _a === void 0 ? void 0 : _a.toString(),
                icon: ((_b = formValues[2]) === null || _b === void 0 ? void 0 : _b.toString()) || productCategory.defaultIcon,
            });
            if (typeof result === "string") {
                openDialogForm(player, {
                    title: "修改失败",
...
// 打开商品类别列表表单
function openCategoryListForm(player, page = 1) {
    const form = new ActionFormData();
    form.title("§w商品类别列表");
    const categories = productCategory.getCategories();
    const pageSize = 10;
    ...
    currentPageCategories.forEach((category) => {
        form.button(`§w${category.name}\n§7${category.description || "无描述"}`, category.icon || "textures/ui/icon_recipe_item");
    });
...
//# sourceMappingURL=CategoryForms.js.map