...
import { extractStackData, generateStackData } from "../../utils/utils";
/**
 * 玩家商店模块
 */
class PlayerShop {
    constructor() {
        system.run(() => {
            this.db = new Database("playerShop");
        });
    }
    // 上架物品
    listItem(player, itemData, cb) {
        // 生成唯一ID
        const id = `${player.name}_${Date.now()}`;
        // 生成可序列化堆栈对象
        const _stackData = extractStackData(itemData.item);
        // 从玩家背包中移除对应数量的物品
        system.run(() => {
    ...
    // 下架物品
    unlistItem(player, itemId, cb) {
        if (!this.db.has(itemId))
            return "物品不存在";
        const item = this.db.get(itemId);
        if (item.seller !== player.name)
            return "只能下架自己的物品";
        ...
        system.run(() => {
            ...
            const _stackItem = generateStackData(item.item);
    ...
    // 获取玩家上架的所有物品
    getPlayerListedItems(playerName) {
...
//# sourceMappingURL=PlayerShop.js.map