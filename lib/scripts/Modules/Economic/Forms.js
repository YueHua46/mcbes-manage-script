...
// 官方商店界面（使用ChestUI）
function openOfficialShopForm(player, page = 0) {
    ...
    chestForm.show(player).then((response) => {
        ...
        if (selection !== undefined && itemPrices[selection]) {
            ...
            if (!enconomic.hasEnoughGold(player.name, item.price)) {
                openDialogForm(player, {
                    title: "购买失败",
                    desc: color.red(`你的金币不足！需要 ${item.price} 金币，但你只有 ${enconomic.getWallet(player.name).gold} 金币。`),
                }, () => openOfficialShopForm(player, page));
                return;
            }
            // 扣除金币
...
// 玩家商店界面（暂未实现）
function openPlayerShopForm(player) {
    const form = new ActionFormData();
    form.title("§w玩家商店");
