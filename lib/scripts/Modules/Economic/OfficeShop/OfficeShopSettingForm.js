...
import { emojiKeyToEmojiPath, emojiPathToEmojiKey, getItemDisplayName, getItemDurability, hasAnyEnchantment, } from "../../../utils/utils";
...
class OfficeShopSettingForm {
    // 官方商店设置主菜单
    openOfficeShopSettingMainMenu(player) {
    ...
    openAddCategoryForm(player) {
        ...
        modal.show(player).then((res) => {
            ...
            if (typeof name !== "string" || typeof description !== "string" || typeof iconIndex !== "number") {
                openDialogForm(player, { title: "失败", desc: "请输入完整且正确的信息" }, () => this.openAddCategoryForm(player));
                return;
            }
    ...
    openCategoryManage(player, categoryName) {
    ...
    // 处理分类管理
    openCategoryManageForm(player, categoryName) {
        const category = officeShop.getCategory(categoryName);
        if (!category) {
            openDialogForm(player, {
                title: "错误",
    ...
    openEditCategoryForm(player, categoryName) {
        ...
        modal.show(player).then((res) => {
            ...
            if (typeof newDescription !== "string" || typeof newIconIndex !== "number") {
                openDialogForm(player, { title: "失败", desc: "请输入完整且正确的信息" }, () => this.openEditCategoryForm(player, categoryName));
                return;
            }
            // 处理编辑逻辑
    ...
    // 处理分类删除
    openDeleteCategoryForm(player, categoryName) {
        const category = officeShop.getCategory(categoryName);
        if (!category) {
            openDialogForm(player, {
                title: "错误",
    ...
    // 处理商品（物品）管理
    openCategoryItemManageForm(player, categoryName) {
        const category = officeShop.getCategory(categoryName);
        if (!category) {
            openDialogForm(player, {
                title: "错误",
    ...
    // 处理商品（物品）列表
    openCategoryItemListForm(player, categoryName, page = 1) {
        const category = officeShop.getCategory(categoryName);
        if (!category) {
            openDialogForm(player, {
                title: "错误",
    ...
    // 商品管理表单
    openItemManageForm(player, categoryName, item) {
    ...
    // 编辑商品表单
    openEditItemForm(player, categoryName, item) {
        const displayName = getItemDisplayName(item.item);
        const title = {
            rawtext: [
                {
        ...
        form.show(player).then((res) => {
            ...
            // 更新商品信息
            officeShop.updateItemMeta(item.data, Object.assign(Object.assign({}, item.data), { amount: newAmount, price: newPrice }));
            openDialogForm(player, {
                title: "成功",
                desc: "商品信息已更新",
            }, () => this.openCategoryItemListForm(player, categoryName));
    ...
    // 删除商品确认表单
    openDeleteItemConfirmForm(player, categoryName, item) {
        const displayName = getItemDisplayName(item.item);
        const body = {
            rawtext: [
                {
    ...
    // 处理添加商品
    openAddCategoryItemForm(player, categoryName) {
        const category = officeShop.getCategory(categoryName);
        if (!category) {
            openDialogForm(player, {
                title: "错误",
        ...
        for (let i = 0; i < container.size; i++) {
            ...
            if (item) {
                ...
                const lores = [`§e数量: §f${item.amount}`, `§e耐久: §f${durability}`];
                chestForm.button(i, getItemDisplayName(item), lores, item.typeId, item.amount, durability, hasAnyEnchantment(item));
            }
        }
        // 添加返回按钮
    ...
    // 商品添加详情表单
    openItemAddDetailsForm(player, categoryName, item, slot) {
        const form = new ModalFormData()
...
//# sourceMappingURL=OfficeShopSettingForm.js.map