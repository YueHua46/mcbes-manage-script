...
export class Economic {
    constructor() {
        ...
        system.run(() => {
            this.db = new Database("eco_wallets");
            this.logDb = new Database("eco_transactions");
        });
    }
    static getInstance() {
    ...
    // 添加金币
    addGold(playerName, amount, reason = "系统奖励") {
        if (amount <= 0)
            return false;
    ...
    // 扣除金币
    removeGold(playerName, amount, reason = "系统消费") {
        if (amount <= 0)
            return false;
        const wallet = this.getWallet(playerName);
    ...
    // 检查余额是否足够
    hasEnoughGold(playerName, amount) {
        return this.getWallet(playerName).gold >= amount;
    }
    // 转账
    transfer(fromPlayer, toPlayer, amount, reason = "转账") {
        if (amount <= 0)
            return "无效金额";
    ...
    // 记录交易日志
    logTransaction(from, to, amount, reason) {
        const transaction = {
            timestamp: Date.now(),
            from,
            to,
    ...
    // 设置玩家金币
    setPlayerGold(playerName, amount) {
        if (amount < 0)
            return false;
        const wallet = this.getWallet(playerName);
        wallet.gold = amount;
