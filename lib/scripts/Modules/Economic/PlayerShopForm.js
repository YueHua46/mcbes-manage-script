...
import shop from "./PlayerShop";
// 玩家商店主界面
export function openPlayerShopForm(player) {
...
// 个人物品管理界面
export function openPersonalItemManageForm(player) {
    const form = new ActionFormData();
    form.title("§w个人物品管理");
...
// 上架个人物品界面
export function openPersonalItemListForm(player) {
    ...
    for (let i = 0; i < container.size; i++) {
        ...
        const durability = item === null || item === void 0 ? void 0 : item.getComponent(ItemComponentTypes.Durability);
        if (item) {
            chestForm.button(i, item.nameTag, [`§7物品类别：${item.typeId.split(":")[1] || item.typeId}`, `§7数量: ${item.amount}`], item.typeId, item.amount, durability === null || durability === void 0 ? void 0 : durability.damage, false);
        }
    }
    // 添加返回按钮
...
// 上架物品确认表单
function openListItemConfirmForm(player, item, slot) {
    ...
    form.show(player).then((data) => {
        ...
        const amount = parseInt(formValues[2]);
        const description = formValues[3];
        // 物品名称是必选
        if (!displayName.trim()) {
            return openDialogForm(player, {
                title: "上架失败",
...
// 下架个人物品界面 - 使用ChestUI
export function openPersonalItemUnlistForm(player, page = 1) {
    // 获取该玩家上架的所有物品
    ...
    for (let i = 0; i < currentPageItems.length; i++) {
        ...
        chestForm.button(i, item.displayName, [
            `§7价格: §e${item.price} 金币`,
            `§7数量: §a${item.amount}`,
            `§7上架时间: §f${item.listTime}`,
            item.description ? `§7描述: §f${item.description}` : "",
            "§7点击下架",
        ], item.item.typeId, item.amount);
    }
    // 添加导航按钮
    if (page > 1) {
...
// 下架物品确认表单
function openUnlistItemConfirmForm(player, item, returnCallback) {
    const form = new ActionFormData();
    form.title("下架物品确认");
    form.body(`您确定要下架以下物品吗？\n\n` +
...
export function openAllListedItemsForm(player, page = 1) {
    // 获取所有上架的物品
...
// 购买物品确认表单
function openBuyItemConfirmForm(player, item, returnCallback) {
    const form = new ActionFormData();
    form.title("购买物品确认");
    form.body(`您确定要购买以下物品吗？\n\n` +
        `${color.yellow("物品名称:")} ${color.green(item.displayName)}\n` +
        `${color.yellow("价格:")} ${color.green(item.price + "")}\n` +
        `${color.yellow("数量:")} ${color.green(item.amount + "")}\n` +
...
//# sourceMappingURL=PlayerShopForm.js.map